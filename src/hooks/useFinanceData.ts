...
export const useFinanceData = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [categoriesData, limitsData, cardsData, transactionsData] =
          await Promise.all([
            getCategories(),
            getCategoryMonthlyLimits(),
            getCreditCards(),
  ...
  const setCategoryMonthlyLimit = useCallback(
  ...
  const addTransaction = useCallback(
    async (transaction: Transaction | Transaction[]) => {
      const newTransactions = Array.isArray(transaction)
        ? transaction
        : [transaction];
  ...
  const getPreviousMonthBalance = useCallback(
    (month: string): number => {
  ...
  const getCreditCardBalance = useCallback(
    (cardId: string, month: string): number => {
      return transactions
        .filter(
          (t) =>
  ...
  const isCategoryOverLimit = useCallback(
    (categoryId: string, month: string): boolean => {
  ...
  const getCategoryLimitSync = useCallback(
